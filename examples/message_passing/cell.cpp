...
// Manual references: lines 18-49, and 54-55 (Actor.tex)
...
behavior unchecked_cell(stateful_actor<cell_state>* self) {
  return {
    ...
    others >> [=](const message& unexpected) {
      aout(self) << "Unexpected message in unchecked_cell: "
                 << unexpected << endl;
    }
  };
}
...
int main() {
  ...
  anon_send_exit(cell1, exit_reason::kill);
  // get an unchecked cell and send it some garbage
  anon_send(cell2, "hello there!");
  anon_send_exit(cell2, exit_reason::kill);
}
